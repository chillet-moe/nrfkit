...
`run --oracle <id>` is the complete daily reproduction entry point after preparation. It obtains the official build toolchain from the validated source receipt, reports that toolchain separately from west and the locked independent GDB during doctor preflight, performs a pristine reference build through the same internal implementation as `reference build`, audits the generated manifest, and then proceeds through guarded programming, serial-ready, reset, and exact-token verification. An explicit `--manifest` selects the firmware-provider path and intentionally omits doctor/build; reports distinguish the two forms by their recorded stages. Standalone `doctor` requires `--official-toolchain` so it cannot silently borrow whichever compiler happens to be in the environment.
...
`p0-gate` is the aggregate acceptance entry point. It invokes the same public `run` and `gdb-smoke` commands for three consecutive hello-world passes, the Bare Metal plus S115 oracle, and token-verified GDB smoke. An already-disabled MSD interface is the preferred state for a probe with the confirmed SEGGER MSD/VCOM limitation, and the gate makes no persistent change in that state. `probe-msd disable --authorize-persistent-change` implements the separately authorized one-time workaround with a full local backup and post-reboot readback; `probe-msd enable --authorize-persistent-change` is the separately authorized reversal. If MSD remains enabled, `p0-gate` refuses to change it unless `--authorize-temporary-msd-disable` is present. That compatibility path remains transactional and restores MSD on success, failure, or interruption. Codex requires separate user authorization before either persistent command or the temporary flag.

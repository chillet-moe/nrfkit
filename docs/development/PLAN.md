...
2026-09-18 新接入的 BLU939 已有仓库自有 PySerial 控制器；运行时不使用或分发厂商
C++ SDK。公共命令完成唯一设备发现、完整元数据读取及 20 ms 被动采样，实收 2001 个
四字节样本并生成标准 `time_s,current_a` CSV；全过程未开启输出或修改电压。设备元数据
报告 `Calibrated=0`，但厂商换算路径不读取该字段，而是直接应用同时返回的六组
电阻/偏移系数；该字段本身既不能证明也不能否定校准。当前 DUT 供电接线尚未记录，
因此这次烟测只证明串口采集链路，不能单独作为 M7 电流、功率或能量验收。新增与相关
回归后的 host suite 为 179 项通过。
